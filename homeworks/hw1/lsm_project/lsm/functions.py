...
def get_lsm_description(
    ...
    if not (type(abscissa) is list):
        abscissa = list(abscissa)
    if not (type(ordinates) is list):
...
def get_lsm_lines(
    ...
    if type(lsm_description) is not LSMDescription:
        raise TypeError
    line_predicted = [(lsm_description.incline*x +
                       lsm_description.shift) for x in abscissa]
...
def get_report(
    ...
    report = '\n'.join([
        ...
        "[INFO]: shift error: " +
        f'{lsm_description.shift_error:.{PRECISION}f}' + ";",
        "\n" + "="*100
    ])
...
def _is_valid_measurments(measurments: list[float]) -> bool:
    ...
    if len(measurments) <= 2:
        raise ValueError
    if not (type(measurments) is list):
        raise TypeError
...
def _get_lsm_description(
    ...
    sigma_y2 = sum((ordinates[i] - a*abscissa[i] - b)**2
